...
アラームがトリガーされると、自動でインスタンスの停止や再起動または EC2 AutoScaling の有効化をすることが出来るため、リソース配置の自動化が可能になります。


次の章からは、利用シーンに合わせた CloudWatch の活用事例をご紹介します。

==[nonum]ダッシュボードを活用した複合的なメトリクスの監視
　ダッシュボードを利用した監視の例を見てみましょう。
...
#@# ==[nonum]CloudWatch アラームと CloudWatch Events による AWS リソースの最適化
...
サーバ内に再起動スクリプトなどを仕込まなくても AWS が裏側でインスタンスに対してアクションを実行してくれるので特別な仕組みの導は入りません。
