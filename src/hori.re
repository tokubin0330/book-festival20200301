...
//list[viewer_packetpolicy][バケットポリシー]{
...
今回、WordPressの静的化の検証実験を行なうに当たって、メンバーが個人的に運用しているサイトを実際に使う事にしました。

現在、このサイトは現在静的ファイルで運用されているので、実際にアクセスして頂いてスピード感などを実感して頂けたらと思います。

=== 検証実験詳細

静的化を行なったサイトを8月1日〜8月15日まで静的化を行なって、S3の運用費を測定したところ以下のようになりました。

✳︎S3のキャプチャーを挿入

=== ec2・レンタルサーバーとの比較

今回検証に使ったサイトは元々ec2のインスタンスを使って運用されていました。
