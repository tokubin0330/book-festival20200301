...
私自身、WordPressでの開発・運用を10年近くに渡って携わって来ましたけど、ひたすらG社のレンタルサーバーを愛用して参りました。
 
その理由はただ一つ！！
 
「安いから！！」「楽だから！！」「ボタン１つでWordPressインストールできるから！！」
 ...
 * 【セキュリティ】
 @<br>{}　
 静的サイトは基本的にセキュリティ面では安全です。Worpressを静的サイトにした場合でも、WordPressで使われるプラグインも全て静的なファイルに書き換えられるので、安全性が高くなります。
 ...
 * 【コスト】
 @<br>{}　
 要求を処理するために必要なサーバーの通信時間は少なくなるので、静的にした方が安くなります。特にトラフィックが多い場合は、コスト削減の恩恵を沢山受けられるに違いありません！！！

ポートフォリオや個人的なWebサイトを運営している場合は、HTMLとCSS（そして多少のJavaScript）のみで構成されているとすると、その場合も静的化をする事によって多くのメリットを得る事ができます。

この章の後半では、実際に静的にした場合に、どれぐらいのコスト削減ができるかの検証結果を掲載します。
...
以下の手順でDocker環境を作っていきます。
 ...
 2. そのディレクトリに移動します。
 3. 以下に示す内容でdocker-compose.ymlというファイルを作成します。
...
静的にすることはこれらの欠点を最適化するための素晴らしい方法です。
そしてそれをS3でホストすることはさらにコストを削減し、よりきめ細かいセキュリティ設定を可能にします。
...
また、私が良く使っているG社のレンタルサーバーと比較してみると以下のような結果になります。
@<br>{}
【1ドル108円で計算】
