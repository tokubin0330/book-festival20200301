...
//list[ec2][ec2との比較]{
  * ec2で運用した場合と比較して静的化した場合は月間で約98%のコスト削減に成功する事が確認できました。
//}
...
//list[rental][レンタルサーバーとの比較]{
  * ec2で運用した場合と比較して静的化は約%のコスト削減に成功する事が確認できました。
//}
...
//list[S3_cost][S3のコスト例]{
  * GET Requestsのコスト　$0.004/10000request
  * 

  もし月間に10万PVのサイトを運営したとしてもかかるS3のコストは$1.96/月、約200円/月になります。
...
WordPressの静的化に関しては以下のような静的化ホスティングを可能にするサービスが出て参りました。
...
//list[hardpress][Hardpress]{
  ...
  * hardpressの特徴としては
  静的化サイトでありながらも、問い合わせフォーム(ContactForm7)やサイト内検索機能や他のプラグインを利用する事ができる
//}
