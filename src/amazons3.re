...
==[nonum] どんなサービス？
Amazon S3(以下S3)は、Amazon S3はAmazon Simple Storage Serviceの略になります。
S3の特徴としては、データを3箇所以上に冗長化して保存されるので99.999999999% という高い耐久性が実現できます。
...
==[nonum] 一緒に使われるサービス
S3と関連するサービスとしてはCloudFrontがあげられます。CloudFrontはCDN(キャッシュ)のサービスです。
コンテンツを海外に配信する場合や、国内でも大量のデータを配信する場合などに利用します。

また、Lambda を使用してS3からの通知を処理することができます。オブジェクトが作成または削除されたときに、イベントをS3からLambda 関数に送信できます。
