...
一般的にはこのマージ前に開発リーダや、上級エンジニアの方のコードレビューを行い、承認が通ったコードだけマージさせることが多いでしょう。
CI もそのようなフローに合わせて自動でマージさせずコードレビューのための承認フローを入れます。

　また、マージされるソースコードは、CI によってコードの規約チェックや Unit テストを行うジョブをくぐり抜けてきています。
したがって、開発リーダのコードレビューは「ソースコード上のビジネスロジックの確認に専念」できます。
「インデントが揃ってない」や「使ってない変数がある」など、コードレビュー時の本質外のマージリクエストの突き返しがなくなります。
開発リーダがコードレビューを行った後は「マージ」ボタンを押して開発用の共通ブランチにマージして終了です。
