...
　継続的デリバリー（以下 CD と呼ぶ）は主に開発コードにバグがないか自動的にテストを行いレポジトリに
アップロードするまでの一連のプロセスの自動化を意味します。#@# 参照 CI/CD全体の範囲図
CI の時と違う点は、CD がコードを最終的にアップロードする先が
本番環境へデプロイされるために用意されたレポジトリである点です。本番環境へデプロイする運用者は CI によってアップロードされた最終的な
ソースコードが保管されているレポジトリさえ確認すれば開発者の変更コードを本番環境へデプロイすることができます。
