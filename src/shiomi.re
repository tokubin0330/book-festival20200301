...
== CI/CDとは
CI/CD という言葉は今や IT 業界では一般的になっており、CI/CD が何を示しているのかを説明する機会が減ってきています。
またプロジェクト毎や会社毎または、個人の認識の違いで CI/CD が取りうる範囲が少し違うこともしばしばあります。
ここではなるべくそういった言葉の齟齬をなるべくなくすために CI/CD が取り扱う範囲を定義します。

 CI/CD は英語では「Continuous Integration/Continuous Delevary」日本語に訳したら「継続的インテグレーション/継続的デリバリー」といいます。
...
CI は最終的にプロジェクトで利用している共通のレポジトリに自動で統合されるところまでを範囲とします。
...
継続的デリバリー（以下 CD と呼ぶ）は主に開発コードにバグがないか自動的にテストを実行し、レポジトリにアップロードするまでの一連のプロセスの自動化を意味します。
...
では、CI/CD のフローで実行される各ジョブについてみていきましょう。
...
=== コードの静的解析
先程のプルリク/マージリクエストイベントを検知するとコードの静的解析を実行します。
...
初期化されていない変数はないか、コードエラーが出ていないかなど様々なコードやプログラム言語に関するチェックを実行します。
...
=== Unit テスト
コードの静的解析が成功すると次に Unit テストを実行します。
...
=== コードのマージ
Unit テストが成功すると開発用の共通ブランチにマージできます。
一般的にはマージ前に開発リーダや、上級エンジニアの方のコードレビューを実行し、承認が通ったコードだけマージさせることが多いでしょう。
CI もそのようなフローに合わせて自動でマージさせずコードレビューのための承認フロー（マージブロック）を設定します。
マージブロックはレポジトリサービス標準で備わっている機能です。
マージリクエストに対してレビュアーをアサインさせレビュアーの承認がないとマージできないようにも設定できます。
...
PHP であれば composer install でフレームワークや依存モジュールをパッケージから展開する処理をここで実行します。
...
このビルドによる環境の差を防ぐために同一ビルドプロセスでビルドを行うことが推奨されます。


複数のデプロイ先を定義するために重要なものが@<strong>{「環境変数」}です。
デプロイの自動化プロセスをうまく回すためにはこの環境変数が重要になります。
ビルドプロセスでは環境に依存したコードを含めずにビルドを実行し、デプロイプロセス時に環境変数によってデプロイ先を分離できます。
...
他にも TravisCI や Jenkins など様々な CI サービスがありますので、プロジェクトにあった CI ツールを選択します。
...
GitLabRunner は CD として AWS サービスを操作できないので S3 バケットにデリバリーした後、AWS CodeDeploy サービスなどを利用して
S3 バケットから各 AWS クラウドサービスにデプロイする CI/CD 環境を作成することがよくあります。
今回も同様に S3 バケットにデリバリーするところまで GitLabRunner で実行します。
...
==== テンプレート
.lint_template や .deploy_template は各 job で利用する共通の処理をテンプレート化できます。
これは gitLabRunner の機能ではなく yaml の機能で、エイリアスを定義しておくと任意の場所に yaml の記述をマージできます。
...
==== cache 
cache は各 job 毎のキャッシュです。次のジョブに作成したコンテンツを引き渡したい場合にこのキャッシュを利用できます。
...
//}
新しいバージョンのインスタンスを徐々に入れ替えていくことで最新バージョンのデプロイの失敗リスクを最小限に抑えアップデートができます。
...
//image[blue-green][ブルー・グリーンデプロイ][scale=0.8]{
//}
...
CodeStar はアプリケーションのコーディングから CI/CD パイプラインをプロジェクトテンプレートの中から選択し、数分で設定できます。
...
CodeStar では数十種類のプロジェクトテンプレートから簡単にアプリケーションと CI/CD 環境を構築できます。
...
別ウィンドウで CodeStar と GitHub の連携認証が求められるので認証を実行します。
...
ここで EC2 のスペックを編集できますので任意で変更します。
...
CodeStar の管理画面ではプロジェクトごとに CI/CD の進行状況や成功・失敗状況、プロジェクトの CI/CD 用の Wiki の編集、サーバの情報などを管理できます@<img>{codestar-9}。
また Code4兄弟の CI/CD 状況を見るためのメニューも用意されているので CodeStar 管理画面内で実行したい操作がすべて完結できるのが特徴です。
//image[codestar-9][CodeStar 管理画面][scale=0.8]{
//}

最後に、作成した GitHub のレポジトリに対してコードを変更して push してみましょう。
すると、管理画面では CI/CD パイプラインが「Source」「Build」「Deploy」の順で進行していく様子が見られます。
Deploy が完了するとインスタンスに新しいバージョンのソースコードが反映されます。

これで初めての CI/CD 環境を簡単に構築できました。
...
CodeBuild や CodeDeploy は様々なプロジェクトに対応できるためにたくさんの設定項目を定義できます。
...
もしそうなっていないなら少しずつでいいですのでプロジェクトに導入してみてください。
